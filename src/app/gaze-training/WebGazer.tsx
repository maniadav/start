    ...
    const initWebGazer = async () => {
      try {
        ...
        if (window.webgazer) {
            ...
            .setGazeListener((data: any, elapsedTime: number) => {
              if (isSurvey) {
                if (data) {
                  ...
                  console.log(
                    `Gaze Data: X=${xprediction}-${(
                      (xprediction / screenWidth) *
                      100
                    ).toFixed(2)}%, Y=${yprediction}-${(
                      (yprediction / screenHeight) *
                      100
                    ).toFixed(2)}%, Time=${elapsedTime}`
                  );
                } else {
          ...
          window.webgazer.showPredictionPoints(false); // Hide the dots
