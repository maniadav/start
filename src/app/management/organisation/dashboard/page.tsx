...
import { getOrganizations, getSurveys, getFiles } from "@management/lib/data-service"
...
export default function OrgDashboard() {
  ...
  const orgSurveys = surveys.filter((s) => s.organizationId === user?.organizationId)
  const orgFiles = files.filter((f) => f.organizationId === user?.organizationId)
  ...
  const stats = [
    ...
    {
      title: "Total Size",
      value: `${Math.round(orgFiles.reduce((acc, f) => acc + f.size, 0) / 1024 / 1024)}MB`,
  ...
  return (
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      <div className="flex items-center space-x-2">
        <SidebarTrigger />
        <div>
          <h2 className="text-3xl font-bold tracking-tight">Organisation Dashboard</h2>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Latest surveys in your organization</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {orgSurveys.slice(0, 5).map((survey) => (
                <div key={survey.id} className="flex items-center space-x-4">
                  ...
                  <div className="text-sm text-muted-foreground">{new Date(survey.createdAt).toLocaleDateString()}</div>
                </div>
              ))}
            </div>
          </CardContent>
