...
import { getObservers, saveObservers, getFiles, formatFileSize } from "@management/lib/data-service"
...
export default function ObserversPage() {
  ...
  const [observers, setObservers] = React.useState(
    getObservers().filter((o) => o.organizationId === user?.organizationId),
  )
  const [filters, setFilters] = React.useState<FilterOptions>({})
  const [isCreateDialogOpen, setIsCreateDialogOpen] = React.useState(false)
  const [isEditDialogOpen, setIsEditDialogOpen] = React.useState(false)
  const [editingObserver, setEditingObserver] = React.useState<Observer | null>(null)
  ...
  const filteredObservers = React.useMemo(() => {
    ...
    if (filters.status && filters.status.length > 0) {
      filtered = filtered.filter((observer) => filters.status!.includes(observer.status))
    }

    if (filters.dateRange?.from || filters.dateRange?.to) {
      filtered = filtered.filter((observer) => {
        const observerDate = new Date(observer.createdAt)
        if (filters.dateRange?.from && observerDate < filters.dateRange.from) return false
        if (filters.dateRange?.to && observerDate > filters.dateRange.to) return false
        return true
      })
    }
  ...
  const columns: ColumnDef<Observer>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const observerId = row.getValue("id") as string
        const fileCount = files.filter((f) => f.observerId === observerId).length
        return fileCount
      },
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const totalSize = observerFiles.reduce((acc, file) => acc + file.size, 0)
  ...
  const handleCreateObserver = () => {
    if (!observerName || !observerEmail || !user) {
      toast({
        title: "Error",
    ...
    setObservers(updatedObservers.filter((o) => o.organizationId === user.organizationId))
        ...
        <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
          ...
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>Add a new observer to your organization.</DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4">
              ...
              <div className="grid gap-2">
                ...
                <Select value={observerStatus} onValueChange={(value: Status) => setObserverStatus(value)}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
      ...
      <Card>
        <CardHeader>
          ...
          <div className="flex items-center gap-2">
            <AdvancedFilters filters={filters} onFiltersChange={setFilters} showStatusFilter showDateFilter />
          </div>
        </CardHeader>
        <CardContent>
          <DataTable
